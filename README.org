...
- =operator[]()=, =at()=, =front()=, and =back()= do not perform bounds
  checking.
- Define =VECTOR_ENABLE_BOUNDS_CHECKS= before including =Vector.h= to enable
  assertion-backed access checks in those methods, or override
  =VECTOR_ACCESS_CHECK(condition)= with a custom hook.
...
python3 tools/pio_task.py upload --example examples/VectorTester --env <env> --port /dev/ttyACM0
#+END_SRC
